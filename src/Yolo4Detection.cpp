...
namespace tk { namespace dnn {
    ...
    bool Yolo4Detection::init(const std::string& tensor_path, const int n_classes, const int n_batches, const float conf_thresh) {
...
void Yolo4Detection::preprocess(cv::Mat &frame, const int bi){
...
void Yolo4Detection::postprocess(const int bi, const bool mAP){
    //get yolo outputs
    if(netRT->yolo_plugins.size() < 2){
    ...
    for(int j=0; j<nDets; j++) {
        ...
        float x0 = (b.x-b.w/2.) / float(netRT->input_dim.w);
        float y0 = (b.y-b.h/2.) / float(netRT->input_dim.h);
        float w = b.w / float(netRT->input_dim.w);
        float h = b.h / float(netRT->input_dim.h);
        // float x1 = (b.x+b.w/2.);
        // float y1 = (b.y+b.h/2.);
...
tk::dnn::Yolo* Yolo4Detection::getYoloLayer(int n) {
    if(n<3)
        return yolo[n];
    else 
        return nullptr;
